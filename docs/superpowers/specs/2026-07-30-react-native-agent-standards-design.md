...
- 同步脚本除共享区块外保持目标 `AGENTS.md` 内容不变;这是结构保护,不代表 marker 外正文免于语义审查。同步后必须对照当前代码、类型、测试、依赖、有效文档与已批准规格清理失效或冲突说明。
- 重复运行结果完全一致。
...
- 在四仓工作树执行 agents-only 同步并逐仓 review diff;另审查 marker 外正文,区分已落地事实与已批准但尚未实现的开发约束。
